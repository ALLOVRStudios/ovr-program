...
use crate::constants::{ALLOVR_AOVR_STAKE_POOL_REGISTRY_PREFIX, ALLOVR_AOVR_STAKE_PREFIX};
use crate::errors::AllovrError;
use crate::state::{StakePoolRegistry, StakeMetadata, StakePool};

#[derive(Accounts)]
#[instruction(amount: u64)]
pub struct RequestStakeWithdrawal<'info> {
    #[account(        
...
pub fn handle_request_stake_withdrawal(
    ctx: Context<RequestStakeWithdrawal>,
    amount: u64
) -> Result<()> {

    require_gt!(amount, 0, AllovrError::MinimumStakeWithdrawalAmountMoreThanZero);
    ...
    let registered_pool = registered_pool_option.unwrap();
    require_keys_eq!(registered_pool.pool_address, ctx.accounts.stake_pool.key(), AllovrError::InvalidPoolIndex);
    
    // get stake holder balance to see max that can be withdrawn
    let mut staked = stake_pool.stakes[usize::from(stake.slot_index)];
    ...
    if registered_pool.total_owed > 0 {
        ...
        let owed_to_stake_holder = share_of_total_percentage * registered_pool.total_owed;        
        staked += owed_to_stake_holder;
    }

    require_gte!(staked, amount, AllovrError::WithdrawalAmountExceedsStakedAmount);
